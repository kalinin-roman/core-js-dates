...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const difference = dayEnd - dayStart;
  const oneDayInMs = 1000 * 60 * 60 * 24;
  return Math.floor(difference / oneDayInMs) + 1;
}
...
function isDateInPeriod(date, period) {
  const dateForVerify = new Date(date);
  const start = new Date(period.start);
  const end = new Date(period.end);

  return dateForVerify >= start && dateForVerify <= end;
}
...
function getWeekNumberByDate(date) {
  ...
  const additionalDays = year.getDay() ? year.getDay() - 1 : 6;
  const timeInMili = date - year;
...
function getNextFridayThe13th(date) {
  let fridayThe13th = getNextFriday(date);
  while (fridayThe13th.getDate() !== 13) {
    fridayThe13th = new Date(
      fridayThe13th.setDate(fridayThe13th.getDate() + 7)
    );
  }
